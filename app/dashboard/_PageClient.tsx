...
export default function DashboardPage() {
  ...
  const [activeTab, setActiveTab] = useState<"lanes" | "roi" | "settings">(
    searchParams.get("tab") === "billing" ? "settings" : "lanes"
  );
  ...
  useEffect(() => {
    ...
    setAnnualUpsellDismissed(localStorage.getItem("annual_upsell_dismissed") === "1");
        ...
        {activeTab === "settings" && (
          <section className="space-y-6">
            ...
            <div className="space-y-3">
              <h2 className="text-sm font-medium uppercase tracking-wider text-muted-foreground">Billing</h2>
              ...
              {/* Annual upsell — shown to active Pro users who haven't dismissed it */}
              {isPro && !annualUpsellDismissed && !annualSwitchMessage && (
                <div className="rounded-lg border border-primary/30 bg-primary/5 p-4 space-y-2">
                  <p className="text-sm font-medium">💡 Switch to annual and save $249</p>
                  <p className="text-xs text-muted-foreground">
                    You are on monthly ($79/mo = $948/yr).<br />
                    Annual plan: $699/yr — saves you $249.
                  </p>
                  <div className="flex gap-2 pt-1">
                    <Button
                      size="sm"
                      disabled={switchingToAnnual}
                      onClick={async () => {
                        trackEvent("annual_upsell_clicked", {});
                        setSwitchingToAnnual(true);
                        try {
                          const res = await fetch("/api/billing/switch-annual", { method: "POST" });
                          const data = await res.json();
                          setAnnualSwitchMessage(data.message ?? "Switched to annual plan successfully!");
                        } catch {
                          setAnnualSwitchMessage("Something went wrong. Please try again.");
                        } finally {
                          setSwitchingToAnnual(false);
                        }
                      }}
                    >
                      {switchingToAnnual ? "Switching…" : "Switch to annual"}
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
                      onClick={() => {
                        localStorage.setItem("annual_upsell_dismissed", "1");
              ...
              {annualSwitchMessage && (
                <p className="text-sm text-emerald-600 font-medium">{annualSwitchMessage}</p>
              )}
            </div>
              ...
              {!isPro ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-border p-4 space-y-3">
                      ...
                      onSubmit={async (e) => {
                        ...
                        try {
                          ...
                          if (res.ok) {
                            ...
                            setInviteSuccess(data.error ?? "Failed to send invite.");
                          }
                        } catch {
                    ...
                    {inviteSuccess && (
                      <p className={`text-xs font-medium ${inviteSuccess === "Invite sent!" ? "text-emerald-600" : "text-destructive"}`}>
                        {inviteSuccess}
                      </p>
                    )}
                  </div>
                          ...
                          <button
                            onClick={async () => {
                              try {
                                ...
                                // silently fail — member list will still reflect the optimistic remove
                              }
                            }}
                  ...
                  {teamMembers.length === 0 && teamLoaded && (
                    <p className="text-xs text-muted-foreground">No team members yet. Invite someone above.</p>
                  )}
                </div>
              )}
            </div>
