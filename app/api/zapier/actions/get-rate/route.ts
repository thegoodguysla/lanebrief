...
// GET /api/zapier/actions/get-rate
// Zapier action: get the current rate for a lane
// Query params: origin, destination, equipment (optional, default dry_van)
export async function GET(request: Request) {
  ...
  if (snapshot.length === 0) {
    return Response.json(
      { error: "No rate data found. Add this lane to your portfolio first." },
      { status: 404 }
    );
  }
